...
// Molde: letras, números y guiones
const REGEX_MOLDE = /^[a-zA-Z0-9\-]+$/;
...
function validarCrearFichaTecnica(data) {
    ...
    // ── 6. Molde (opcional) ────────────────────────────────────────────────────
    if (data.numeroMolde !== undefined && data.numeroMolde !== null && data.numeroMolde !== '') {
        const molde = String(data.numeroMolde).trim();
        if (molde.length > 50) {
            errores.push('El molde no puede superar 50 caracteres');
        } else if (!REGEX_MOLDE.test(molde)) {
            errores.push('El molde solo permite letras, números y guiones (-)');
        }
    }
