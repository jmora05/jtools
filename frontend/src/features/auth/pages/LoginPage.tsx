...
import {
  ...
  ArrowLeftIcon, KeyIcon, ShieldCheckIcon,
} from 'lucide-react';
...
// Muestra MM:SS y llama onExpire cuando llega a 0.
function CountdownTimer({ expiresAt, onExpire }: { expiresAt: Date; onExpire: () => void }) {
  ...
  return <span className={remaining < 60 ? 'text-red-500 font-semibold' : 'text-blue-600 font-semibold'}>{mm}:{ss}</span>;
}
...
export function LoginPage({ onLogin }: { onLogin: (user: any) => void }) {
  ...
  const [recoveryStep, setRecoveryStep]   = useState(1); // 1=email 2=otp 3=nueva-pass 4=éxito
  const [recoveryEmail, setRecoveryEmail] = useState('');
  const [verificationCode, setVerificationCode] = useState('');
  const [newPassword, setNewPassword]         = useState('');
  const [confirmNewPassword, setConfirmNewPassword] = useState('');
  const [codeExpiry, setCodeExpiry]           = useState<Date | null>(null);
  const [resetToken, setResetToken]           = useState('');        // JWT de un solo uso
  const [remainingAttempts, setRemainingAttempts] = useState(5);     // intentos restantes
  const [resendCooldown, setResendCooldown]   = useState(0);         // segundos hasta poder reenviar
  ...
  const handleSendRecoveryEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Expiración local del temporizador (10 min)
      setCodeExpiry(new Date(Date.now() + 10 * 60 * 1000));
      setRemainingAttempts(5);
      setResendCooldown(60); // 60 s antes de poder reenviar
      setRecoveryStep(2);
      // En desarrollo el backend puede devolver devCode si el email falla
      if (resp.devCode) {
        toast.info(`[DEV] Código: ${resp.devCode}`);
      } else {
  ...
  // Paso 2 — verificar OTP y obtener resetToken
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(msg);
      // Extraer intentos restantes del mensaje si viene en el error
  ...
  // Paso 3 — cambiar contraseña con resetToken
  ...
  // Reenviar código — solo disponible cuando el cooldown llega a 0
